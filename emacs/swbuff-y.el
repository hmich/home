...
;; Lxast-Updated: Mon May 21 02:20:52 2007 (7200 +0200)
...
;;             Added C-g to keymap.
;;
;; 2005-11-29: 0.57, Changed timeout timer to quit instead of select.
;;
;; 2007-05-18: 0.58, Added w for other window, f for new frame.
...
(let ((fmt " f1:help,  w:window, f:frame, RET:switch frame (underlined), other:select."))
  (if (featurep 'xemacs)
...
(defvar swbuff-y-keymap-alist (list (cons 'swbuff-y-keymap swbuff-y-keymap)))
...
(define-key swbuff-y-keymap [(escape)]    'swbuff-y-quit)
(define-key swbuff-y-keymap [(f1)] 'swbuff-y-keyboard-help)
...
(define-key swbuff-y-keymap [(?f)]    'swbuff-y-show-in-new-frame)
(define-key swbuff-y-keymap [(?w)]    'swbuff-y-show-in-other-window)
...
(defun swbuff-y-restore-after-help(key)
  (let ((fun (lookup-key swbuff-y-keymap key)))
    (swbuff-show-status-window)
    (when fun
      (funcall fun))))

(defun swbuff-y-keyboard-help()
  "Give keyboard help for `swbuff-y'.
You can use the following keyboard keys while cycling through the
buffers:

  \\{swbuff-y-keymap}

Other keys will select current choice in the selected window.
"
  (interactive)
  (save-selected-window
    (save-window-excursion
      (describe-function 'swbuff-y-keyboard-help)
      (let* ((key (read-key-sequence "Type any of those keys: ")))
        ;;(message "fun=%s, key=%s" fun key)(sit-for 2)
        (run-with-idle-timer 0 nil 'swbuff-y-restore-after-help key)))))
...
(defvar swbuff-y-other-window nil
  "Used for showing buffer in the other window.")
...
(defun swbuff-y-show-in-new-frame()
  ...
  (swbuff-discard-status-window))
(defun swbuff-y-show-in-other-window()
  (interactive)
  (swbuff-y-check-status-window-active)
  (setq swbuff-y-other-window t)
  (swbuff-discard-status-window))
...
(defun swbuff-y-show-popup ()
  ...
  (let ((menulst nil)
    ...
    (add-to-list 'menulst '("Show selected buffer in &other window" . this-other-window))
    ...
    (cond ((not choice)
          ...
          ((eq choice 'this)
           (swbuff-discard-status-window))
          ((eq choice 'this-other-window)
           (swbuff-y-show-in-other-window))
	  ...
	  (save-window-excursion
	    ...
	    (while (setq buffer (car buffer-list))
          ...
          (cond (swbuff-y-jump-to-window
                ...
                (swbuff-y-other-window
                 (switch-to-buffer-other-window swbuff-current-buffer)
                 ;;(message "Swbuff-y: Switched to buffer in other window!")
                 )
                (swbuff-y-new-frame
                 (set-window-buffer (select-window (frame-first-window (make-frame)))
...
the delay specified by `swbuff-clear-delay'."
  ...
  (setq swbuff-y-other-window nil)
